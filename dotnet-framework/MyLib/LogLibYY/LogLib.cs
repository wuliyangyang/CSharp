...
namespace LogTool
{
    ...
    public class LogTool
    {
        ...
        public LogTool(string fileName)
        {
            ...
            _date = DateTime.Now;
            _dateStr = _date.ToString("yyyy-MM-dd");
            _logFile = Path.Combine(_path, "Log/" + _dateStr + "/" + fileName + ".txt");
            CreateDirectory(_logFile);
        }
        //使用
        ...
        private void UpdateLogPath()
        {
            ...
            _logFile = Path.Combine(_path, "ModuleLog/" + _dateStr + "/" + _moduleName + ".txt");
            CreateDirectory(_logFile);
            ...
            try
            {
                ...
                */
                if (!File.Exists(_logFile))
                {
                    fileStream = fileInfo.Create();
                    writer = new StreamWriter(fileStream);
                    writer.WriteLine(DateTime.Now.ToString("yyyy-MM-dd_HH-mm-ss.fff") + ":Creat...");
                }
                ...
                writer.WriteLine(DateTime.Now.ToString("yyyy-MM-dd_HH-mm-ss.fff") + ":" + info);
                //writer.WriteLine("----------------------------------");
            }
            finally
            {
