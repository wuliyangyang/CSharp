...
namespace YY.SOA.WebApi.Controllers
{
    ...
    public class UsersController : ApiController
    {
        ...
        {
            if ("yy".Equals(username) && "123456".Equals(password))//应该数据库校验
            {
                FormsAuthenticationTicket ticketObject = new FormsAuthenticationTicket(0, username, DateTime.Now, DateTime.Now.AddMinutes(2), true, string.Format("{0}&{1}", username, password), FormsAuthentication.FormsCookiePath);
                var result = new
                {
                    Result = true,
